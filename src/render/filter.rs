...
pub fn render(
    ...
    let initial_transform = Transform::from_scale(ctx.options.raster_scale, ctx.options.raster_scale)
        .pre_concat(Transform::from_translate(group_bbox.x() - layer_bbox.x(),
                                              group_bbox.y() - layer_bbox.y()));
    let pixmap_size = Size::from_wh(layer_bbox.width() * ctx.options.raster_scale, layer_bbox.height() * ctx.options.raster_scale)?;
    ...
    resvg::render_node(&Node::Group(Box::new(group.clone())),
                       initial_transform,
                       &mut pixmap.as_mut());
    ...
    let image = usvg::Image {
        ...
        view_box: ViewBox { rect: layer_bbox.transform(
            Transform::from_scale(ctx.options.raster_scale, ctx.options.raster_scale)).unwrap(), aspect: AspectRatio::default() },
    ...
    let group_node = Node::Group(Box::new(
        Group {
            transform: Transform::from_scale(1.0 / ctx.options.raster_scale, 1.0 / ctx.options.raster_scale),
