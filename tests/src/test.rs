...
fn main() -> ExitCode {
    let references: Vec<TestFile> = (&*REF_FILES).iter().map(|f| TestFile::new(f)).collect();
    let svg_files: Vec<TestFile> = (&*SVG_FILES).iter().map(|f| TestFile::new(f)).collect();
    ...
    for svg_file in &svg_files {
        ...
        let expected_image = Reader::open(svg_file.as_references_path()).unwrap().decode().unwrap().into_rgba8();
        let actual_image = svg_to_image(&fs::read_to_string(svg_file.as_svg_path()).unwrap(), &test_runner);
        ...
        if diff {
            ...
            diff_image.save_with_format(svg_file.as_diffs_path(), image::ImageFormat::Png).unwrap();
        }   else {
...
fn print_test_case_result(test_status: TestStatus, file: &TestFile) -> io::Result<()> {
    ...
    match test_status {
        TestStatus::SUCCESS => stdout.set_color(ColorSpec::new().set_fg(Some(Color::Green)))?,
        TestStatus::FAILURE => stdout.set_color(ColorSpec::new().set_fg(Some(Color::Red)))?,
        TestStatus::SKIPPED => stdout.set_color(ColorSpec::new().set_fg(Some(Color::White)))?
    }
