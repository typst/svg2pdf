...
fn main() {
    ...
    let svg_files: Vec<TestFile> = if !args.replace {
        ...
        let existing_svg_references: Vec<TestFile> = (&*REF_FILES).iter().map(|f| TestFile::new(f)).collect();
        (&*SVG_FILES).iter().map(|f| TestFile::new(f)).filter(|f| existing_svg_references.contains(f) ).collect()
    };

    let number_of_svg_files = (&*svg_files).len() as u64;

    if args.replace {
        println!("Regenerating {} of {} reference images...", number_of_svg_files, (&*SVG_FILES).len() as u64);
    }   else {
    ...
    for svg_file in &svg_files {
        ...
        let image = svg_to_image(&fs::read_to_string(input_path.to_str().unwrap()).unwrap(), &test_runner);
